...
public class HttpRequestImpl implements HttpRequest {
    ...
    static {
        OkHttpClient okHttpClientBuild = new OkHttpClient.Builder().dispatcher(getDispatcher()).build();
    ...
    public void executeRequest(HttpResponder httpResponder, long j, String str, String str2, String str3, boolean z) {
        ...
        try {
            ...
            String strBuildResourceUrl = HttpRequestUrl.buildResourceUrl(httpUrl.host().toLowerCase(MapLibreConstants.MAPLIBRE_LOCALE), str, httpUrl.querySize(), z);
            Request.Builder builderAddHeader = new Request.Builder().url(strBuildResourceUrl).tag(strBuildResourceUrl.toLowerCase(MapLibreConstants.MAPLIBRE_LOCALE)).addHeader("User-Agent", userAgentString);
            if (str2.length() > 0) {
                builderAddHeader.addHeader("If-None-Match", str2);
            } else if (str3.length() > 0) {
                builderAddHeader.addHeader("If-Modified-Since", str3);
            }
            Call callNewCall = client.newCall(builderAddHeader.build());
            this.call = callNewCall;
            callNewCall.enqueue(okHttpCallback);
        } catch (Exception e) {
        ...
        public void onResponse(Call call, Response response) throws IOException {
            ...
            try {
                try {
                    ...
                    response.close();
                    this.httpRequest.onResponse(response.code(), response.header("ETag"), response.header("Last-Modified"), response.header("Cache-Control"), response.header("Expires"), response.header("Retry-After"), response.header("x-rate-limit-reset"), bArrBytes);
                } catch (IOException e) {
                    onFailure(call, e);
                    response.close();
                }

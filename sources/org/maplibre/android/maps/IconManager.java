...
class IconManager {
    ...
    private Icon loadDefaultIconForMarker(Marker marker) {
        Icon iconDefaultMarker = IconFactory.getInstance(MapLibre.getApplicationContext()).defaultMarker();
    ...
    void iconCleanup(Icon icon) {
        if (this.iconMap.get(icon) != null) {
            Integer numValueOf = Integer.valueOf(r0.intValue() - 1);
            if (numValueOf.intValue() == 0) {
                remove(icon);
            } else {
                updateIconRefCounter(icon, numValueOf.intValue());
            }
        }
    }

...
class PermittedActionsGuard {
    ...
    boolean isMissingActions(int i, int i2, int i3) {
        long jUpdatePermittedActions = updatePermittedActions(i2, i3);
        long j = i;
        if (j == jUpdatePermittedActions) {
            return false;
        }
        while (jUpdatePermittedActions != 0) {
            if (j == (255 & jUpdatePermittedActions)) {
                return false;
            }
            jUpdatePermittedActions >>= 8;
        }
        return true;
    }

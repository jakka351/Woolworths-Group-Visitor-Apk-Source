...
public class RotateGestureDetector extends ProgressiveGesture<OnRotateGestureListener> {
    ...
    float calculateAngularVelocityVector(float f, float f2) {
        float fAbs = Math.abs((float) (((getFocalPoint().x * f2) + (getFocalPoint().y * f)) / (Math.pow(getFocalPoint().x, 2.0d) + Math.pow(getFocalPoint().y, 2.0d))));
        return this.deltaSinceLast < 0.0f ? -fAbs : fAbs;
    }

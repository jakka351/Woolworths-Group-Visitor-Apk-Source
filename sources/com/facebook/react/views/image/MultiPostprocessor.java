...
public class MultiPostprocessor implements Postprocessor {
    ...
    public CloseableReference<Bitmap> process(Bitmap bitmap, PlatformBitmapFactory platformBitmapFactory) {
        ...
        try {
            Iterator<Postprocessor> it = this.mPostprocessors.iterator();
            CloseableReference<Bitmap> closeableReferenceMo386clone = null;
            while (it.hasNext()) {
                closeableReferenceProcess = it.next().process(closeableReferenceMo386clone != null ? closeableReferenceMo386clone.get() : bitmap, platformBitmapFactory);
                CloseableReference.closeSafely(closeableReferenceMo386clone);
                closeableReferenceMo386clone = closeableReferenceProcess.mo386clone();
            }
            return closeableReferenceProcess.mo386clone();
        } finally {

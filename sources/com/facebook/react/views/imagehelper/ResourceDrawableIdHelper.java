...
public class ResourceDrawableIdHelper {
    ...
    public int getResourceDrawableId(Context context, String str) {
        ...
        String strReplace = str.toLowerCase(Locale.ROOT).replace("-", "_");
        try {
            return Integer.parseInt(strReplace);
        } catch (NumberFormatException unused) {
            synchronized (this) {
                if (this.mResourceDrawableIdMap.containsKey(strReplace)) {
                    return this.mResourceDrawableIdMap.get(strReplace).intValue();
                }
                int identifier = context.getResources().getIdentifier(strReplace, BuildConfig.FLAVOR, context.getPackageName());
                this.mResourceDrawableIdMap.put(strReplace, Integer.valueOf(identifier));
                return identifier;
            }
        }
    }

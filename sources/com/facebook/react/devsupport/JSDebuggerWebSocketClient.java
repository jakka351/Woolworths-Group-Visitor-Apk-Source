...
public class JSDebuggerWebSocketClient extends WebSocketListener {
    ...
    public void loadBundle(String str, HashMap<String, String> map, JSDebuggerCallback jSDebuggerCallback) throws IOException {
        ...
        try {
            StringWriter stringWriter = new StringWriter();
            JsonWriter jsonWriterBeginObject = new JsonWriter(stringWriter).beginObject().name("id").value(andIncrement).name(Request.JsonKeys.METHOD).value("executeApplicationScript").name("url").value(str).name("inject").beginObject();

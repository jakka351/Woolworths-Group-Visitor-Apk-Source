...
public class DidJSUpdateUiDuringFrameDetector implements NotThreadSafeBridgeIdleDebugListener, NotThreadSafeViewHierarchyUpdateDebugListener {
    ...
    public synchronized boolean getDidJSHitFrameAndCleanup(long j, long j2) {
        boolean z;
        boolean zHasEventBetweenTimestamps = hasEventBetweenTimestamps(this.mViewHierarchyUpdateFinishedEvents, j, j2);
        boolean zDidEndFrameIdle = didEndFrameIdle(j, j2);
        z = true;
        if (!zHasEventBetweenTimestamps && (!zDidEndFrameIdle || hasEventBetweenTimestamps(this.mViewHierarchyUpdateEnqueuedEvents, j, j2))) {
            z = false;
        }
        ...
        this.mWasIdleAtEndOfLastFrame = zDidEndFrameIdle;
        return z;
    }

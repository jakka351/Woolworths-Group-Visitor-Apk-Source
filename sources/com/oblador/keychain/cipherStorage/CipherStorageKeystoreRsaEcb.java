...
public class CipherStorageKeystoreRsaEcb extends CipherStorageBase {
    ...
    public void decrypt(DecryptionResultHandler decryptionResultHandler, String str, byte[] bArr, byte[] bArr2, SecurityLevel securityLevel) throws CryptoFailedException {
        ...
        try {
            try {
                Key keyExtractGeneratedKey = extractGeneratedKey(defaultAliasIfEmpty, securityLevel, new AtomicInteger(1));
                try {
                    decryptionResultHandler.onDecrypt(new CipherStorage.DecryptionResult(decryptBytes(keyExtractGeneratedKey, bArr), decryptBytes(keyExtractGeneratedKey, bArr2)), null);
                } catch (UserNotAuthenticatedException e) {
                    e = e;
                    key = keyExtractGeneratedKey;
    ...
    private CipherStorage.EncryptionResult innerEncryptedCredentials(String str, String str2, String str3, SecurityLevel securityLevel) throws GeneralSecurityException, IOException {
        ...
        PublicKey publicKeyGeneratePublic = KeyFactory.getInstance(ALGORITHM_RSA).generatePublic(new X509EncodedKeySpec(keyStoreAndLoad.getCertificate(str).getPublicKey().getEncoded()));
        return new CipherStorage.EncryptionResult(encryptString(publicKeyGeneratePublic, str3), encryptString(publicKeyGeneratePublic, str2), this);
    }
    ...
    protected KeyGenParameterSpec.Builder getKeyGenSpecBuilder(String str, boolean z) throws GeneralSecurityException {
        KeyGenParameterSpec.Builder keySize = new KeyGenParameterSpec.Builder(str, 3).setBlockModes(BLOCK_MODE_ECB).setEncryptionPaddings(PADDING_PKCS1).setRandomizedEncryptionRequired(true).setUserAuthenticationRequired(true).setKeySize(z ? 512 : 2048);

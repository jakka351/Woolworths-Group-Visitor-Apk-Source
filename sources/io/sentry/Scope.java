...
public final class Scope implements IScope {
    ...
    public Session withSession(IWithSession iWithSession) {
        Session sessionM1027clone;
        synchronized (this.sessionLock) {
            iWithSession.accept(this.session);
            sessionM1027clone = this.session != null ? this.session.m1027clone() : null;
        }
        return sessionM1027clone;
    }
    ...
    public SessionPair startSession() {
        ...
        synchronized (this.sessionLock) {
            ...
            if (this.options.getRelease() != null) {
                ...
                sessionPair = new SessionPair(this.session.m1027clone(), session != null ? session.m1027clone() : null);
            } else {
    ...
    public Session endSession() {
        ...
        synchronized (this.sessionLock) {
            ...
            if (this.session != null) {
                this.session.end();
                Session sessionM1027clone = this.session.m1027clone();
                this.session = null;
                session = sessionM1027clone;
            }
        }

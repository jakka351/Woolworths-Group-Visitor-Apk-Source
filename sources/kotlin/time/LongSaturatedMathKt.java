...
public final class LongSaturatedMathKt {
    ...
    public static final long m2565saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM2482toLongimpl = Duration.m2482toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m2564checkInfiniteSumDefinedPjuGub4(j, j2, jM2482toLongimpl);
        }
        if ((1 | (jM2482toLongimpl - 1)) == Long.MAX_VALUE) {
            return m2566saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM2482toLongimpl;
        return ((j ^ j3) & (jM2482toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m2564checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m2468isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m2566saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM2439divUwyO8pc = Duration.m2439divUwyO8pc(j2, 2);
        long jM2482toLongimpl = Duration.m2482toLongimpl(jM2439divUwyO8pc, durationUnit);
        return ((1 | (jM2482toLongimpl - 1)) > Long.MAX_VALUE ? 1 : ((1 | (jM2482toLongimpl - 1)) == Long.MAX_VALUE ? 0 : -1)) == 0 ? jM2482toLongimpl : m2565saturatingAddNuflL3o(m2565saturatingAddNuflL3o(j, durationUnit, jM2439divUwyO8pc), durationUnit, Duration.m2471minusLRDsOJo(j2, jM2439divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m2538getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m2537getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m2489unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m2472plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m2489unaryMinusUwyO8pc(infinityOfSign(j3));
        }
